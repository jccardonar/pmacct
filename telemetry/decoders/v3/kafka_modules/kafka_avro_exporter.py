...
class KafkaAvroExporter(Exporter):
    ...
    def process_metric(self, datajsonstring):
        ...
        self.logger.debug("In process_metric")
        ...
        if "grpcPeer" in jsondata["collector"]["grpc"]:
            ...
            if "encoding_path" in jsondata["collector"]["data"]:
                ...
                self.logger.debug("Found encoding_path: %s" % encoding_path)
                ...
                if avscid is not None:
                    ...
                    self.logger.debug("avroinstance is: %s" % (avroinstance))
                ...
                self.logger.info("%s -> encoding_path is missing" % grpcPeer)
        else:
            self.logger.info("grpcPeer is missing" % jsondata)
    ...
    def getavro_schid_instance(self, avscid):
        # global  self.avscmap
        self.logger.debug("In getavro_schid_instance with avscid: %s" % avscid)
        avroinstance = None
        if avscid in self.avscmap:
            if "avroinstance" in self.avscmap[avscid]:
                self.logger.debug("avroinstance found in dict  self.avscmap")
                avroinstance = self.avscmap[avscid]["avroinstance"]
            else:
                self.logger.debug("avroinstance not found in dict  self.avscmap")
            ...
            self.logger.debug("avroid not found in dict  self.avscmap")
            self.create_avro_schid_instance(avscid)
            if "avroinstance" in self.avscmap[avscid]:
                self.logger.debug(
                ...
                avroinstance = self.avscmap[avscid]["avroinstance"]
        self.logger.debug("I will return the avroinstance: %s" % avroinstance)
        return avroinstance
    ...
    def create_avro_schid_instance(self, avscid):
        ...
        self.logger.info("Creating avroinstance for avro-schemaid: %s" % (avscid))
    ...
    def getavroschema(self, avscid):
        # global  self.avscmap
        self.logger.debug("In getavroschema with avscid: %s" % avscid)
        avsc = None
        if avscid in self.avscmap:
            ...
            # self.logger.info("avsc not found in dict  self.avscmap")
            self.loadavsc(avscid)
            if avscid in self.avscmap:
                if "avsc" in self.avscmap[avscid]:
                    avsc = self.avscmap[avscid]["avsc"]
    ...
    def loadavsc(self, avscid):
        # global  self.avscmap
        self.logger.debug("In loadavsc with avscid: %s" % avscid)
        avsc = None
        self.logger.debug(
        ...
        self.logger.info("SCHEMA_OF_ID(%s): %s" % (avscid, avsc_dict["name"]))
    ...
    def serialize(self, jsondata, topic, avscid, avroinstance):
        self.logger.debug(
            "JSONDATA:%s\nTOPIC:%s\nAVSCID:%s\nAVROINSTANCE:%s\nSERIALIZELOG:%s"
            % (jsondata, topic, avscid, avroinstance, self.logger)
        )
