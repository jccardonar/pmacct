...
class TransformationWarningIf(Transformtionifmatch):
    ...
    def transform(self, metric, warnings=None):
        ...
        if (
            metric.content
            and isinstance(metric.content, dict)
            and metric.content.get(self.key, None) == self.value
        ):
...
class TestTransformation:
    ...
    def test_pipe_transform(self):
        ...
        warning_if_four = TransformationWarningIf("number", 4)
        error_if_three = TransformationErrorIf("number", 3)
        error_if_four = TransformationErrorIf("number", 4)
        error_if_two = TransformationErrorIf("number", 2)
        metric = MetricDummy()

        assert metric.number == None
        state = (
            TransformationState.from_metric(metric)
            .transform(t_multiplier_2)
            .transform(t_sums_1)
            .transform(t_multiplier_2)
        )
        ...
        resulting_metrics = TransformationState.apply_transformation(
            metric, pipeline, on_error=on_error, on_warnings=on_warnings
        )
        assert [x.number for x in resulting_metrics] == [4]
        assert set([x.fields.get("value", None) for x in errors]) == set([3, 2])
        assert [x.fields.get("value", None) for x in all_warnings] == [3, 3, 4]
    ...
    def test_pipe_convert(self):
        ...
        new_state = (
            ...
            .convert(converter)
            .convert(converter)
            .convert_raiser_error(warning_if_two)
            .convert(converter)
        )
        assert new_state.metric
        new_metric = new_state.metric
        ...
        with pytest.raises(WarningNumberMatched):
            MetricState(metric).convert(converter).convert(
                converter
            ).convert_raiser_error(error_if_two).convert(converter)
