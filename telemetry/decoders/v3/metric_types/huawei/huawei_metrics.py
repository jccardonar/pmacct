...
class HuaweCompactToHuaweiElements(TransformationBase):
    def __init__(self, decoder_constructor):
        self.decoder_constructor = decoder_constructor

    def _get_decoder(self, metric):
        try:
            decoder = self.decoder_constructor.get_decoder(metric.module)
        except:
            raise PmgrpcdException(f"Decoder for {metric.module} not found")
        return decoder
    
    def transform(self, metric, warnings=None):
        decoder = self._get_decoder(metric)
        for elem in metric.content:
            ...
            element_content = decoder.decode(elem["content"])
